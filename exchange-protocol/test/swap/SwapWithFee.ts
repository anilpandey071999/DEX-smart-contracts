...
async function addLiquidityNormalERC20() {
    ...
    await KyotoSwapFactory.setFeeTo(feeTo.getAddress());
...
async function addLiquidityRebaseERC20() {
    const {
        ...
        feeToSetter,
        timeStamp
    } = await loadFixture(deployFactoryandRouter)
    ...
    return {
        SimpleTokenERC20_1, rebased, User1, User2, feeToSetter, KyotoSwapRouter, KyotoSwapFactory, timeStampNew
    }
}
...
describe("Swapping ERC20 Tokens For ERC20 Tokens with out Fees", function () {
    ...
    describe("swapping Rebase ERC20 Token", async function () {
        it("swapping Rebase ERC20 Token Success", async function () {
            const { SimpleTokenERC20_1, rebased, User1, User2, feeToSetter, KyotoSwapRouter, KyotoSwapFactory, timeStampNew } = await addLiquidityRebaseERC20()
            ...
            await pair.connect(feeToSetter).setControllerFeeAddress(await User2.getAddress())
            await pair.connect(feeToSetter).setControllerFeeAmount(50)
