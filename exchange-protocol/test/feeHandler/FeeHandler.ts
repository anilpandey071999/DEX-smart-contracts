...
describe("Fee Handler", async () => {
  let owner: Signer,
    ...
    KSFeeHandlerFactory: KSFeeHandler__factory,
    KSFeeHandler: KSFeeHandler,
    busd: SimpleERC20,
    kswap: SimpleERC20,
  ...
  before(async () => {
    ...
    KSFeeHandlerFactory = (await ethers.getContractFactory("KSFeeHandler")) as KSFeeHandler__factory;
    KSFeeHandler = (await KSFeeHandlerFactory.connect(owner).deploy()) as KSFeeHandler;
    UUPSProxyFactory = (await ethers.getContractFactory("UUPSProxy")) as UUPSProxy__factory;
    UUPSProxy = await UUPSProxyFactory.connect(owner).deploy(KSFeeHandler.address, ZERO_ADDRESS, initializeFuncData);
    ...
    KSFeeHandler = KSFeeHandler.attach(UUPSProxy.address);
  ...
  it("Set FeeTo in Factory", async () => {
    await kyotoSwapFactory.connect(owner).setFeeTo(KSFeeHandler.address);
  });
  ...
  it("Process Fee", async () => {
    ...
    await KSFeeHandler.connect(owner).processFee(liquidityList, swapList, false);
    console.log("KSWAP Balance of FeeHandler ", ethers.utils.formatEther(await kswap.balanceOf(KSFeeHandler.address)));
  });
  it("Send KSWAP", async () => {
    const amount = ethers.utils.parseEther("900");
    const burnAmount = amount.mul(await KSFeeHandler.kswapBurnRate()).div(await KSFeeHandler.RATE_DENOMINATOR());
    const vaultAmount = amount.sub(burnAmount);
    expect(KSFeeHandler.connect(owner).sendKSwap(amount)).to.emit(KSFeeHandler, "Transfer").withArgs(KSFeeHandler.address, BURN_ADDRESS, burnAmount);
  });
});
